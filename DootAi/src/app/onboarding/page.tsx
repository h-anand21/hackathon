...
import { signInWithPopup, User as FirebaseUser, signOut, signInWithEmailAndPassword } from "firebase/auth";
...
function OnboardingPageContent() {
  ...
  // Handle Guest / Judge Demo login to bypass testing limitations
  const handleGuestLogin = async () => {
    try {
      const guestMock = {
        uid: "guest-judge-dootai",
        email: "judge@doot.ai",
        displayName: "Hackathon Judge",
        photoURL: "https://api.dicebear.com/7.x/bottts/svg?seed=judge"
      };

      // Sync mock user in PostgreSQL
      await fetch("/api/auth/sync", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          uid: guestMock.uid,
          email: guestMock.email,
          name: guestMock.displayName,
          photoUrl: guestMock.photoURL
        })
      });

      // Sign in through our transparent proxy helper
      if ((auth as any).signInAsGuest) {
        await (auth as any).signInAsGuest(guestMock);
      }
      ...
      setStep(3); // Go directly to dashboard sync simulation
    } catch (error) {
                      ...
                      {!user ? (
                        <div className="flex flex-col space-y-2.5">
                          ...
                          <button
                            ...
                            className="w-full py-2.5 bg-[#ebdcc8]/50 hover:bg-[#ebdcc8]/80 text-[#2b2725] font-handwriting font-black text-sm rounded-xl hover:scale-[1.01] active:scale-[0.99] transition-all flex items-center justify-center space-x-2 cursor-pointer shadow-sm border border-dashed border-[#2b2725]/40"
                          >
                            <span>🌸 Guest/Judge Quick Login</span>
                          </button>
                        </div>
                      ) : (
                        <div className="flex flex-col space-y-2.5">
                          ...
                          {user?.uid === "guest-judge-dootai" && (
                            <button
                              onClick={() => {
                                setConnections({ gmail: true, googlecalendar: true });
                                setStep(3);
                              }}
                              className="w-full py-2.5 bg-[#ebdcc8]/50 hover:bg-[#ebdcc8]/80 text-[#2b2725] font-handwriting font-black text-xs rounded-xl hover:scale-[1.01] active:scale-[0.99] transition-all flex items-center justify-center space-x-1.5 cursor-pointer border border-dashed border-[#2b2725]/30"
                            >
                              <span>🌸 Skip Connections (Demo Mode)</span>
                            </button>
                          )}
                        </div>
                      )}
                        ...
                        <div className="flex flex-col space-y-2">
                          ...
                          {user?.uid === "guest-judge-dootai" && (
                            <button
                              ...
                              className="w-full py-2 bg-[#ebdcc8]/50 hover:bg-[#ebdcc8]/80 text-[#2b2725] font-handwriting font-black text-xs rounded-xl hover:scale-[1.01] active:scale-[0.99] transition-all flex items-center justify-center space-x-1.5 cursor-pointer border border-dashed border-[#2b2725]/30"
                            >
                              <span>🌸 Skip Connections (Demo Mode)</span>
                            </button>
                          )}
