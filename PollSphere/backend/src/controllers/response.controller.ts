...
export const getPublicPoll = async (req: Request, res: ExpressResponse): Promise<void> => {
  try {
    ...
    res.status(200).json({ success: true, poll, questions: formattedQuestions });
  } catch (error: any) {
    res.status(500).json({ success: false, error: error.message });
  }
};

export const submitResponse = async (req: Request, res: ExpressResponse): Promise<void> => {
  try {
    const { shareId } = req.params;
    ...
    if (new Date() > new Date(poll.expiresAt)) {
      ...
      res.status(403).json({ success: false, error: "Poll has expired and is no longer accepting votes." });
      return;
    }
    ...
    if (poll.responseMode === 'authenticated' && !voterId) {
       res.status(401).json({ success: false, error: "You must be logged in to vote on this poll." });
       return;
    }
    ...
    if (voterId) {
      ...
      if (existing) {
        res.status(409).json({ success: false, error: "You have already voted on this poll." });
        return;
      }
    } else {
      const ip = req.ip || req.headers['x-forwarded-for'] || 'unknown';
      const existing = await Response.findOne({ pollId: poll._id, ipAddress: ip });
      if (existing) {
        res.status(409).json({ success: false, error: "Your IP has already voted on this anonymous poll." });
        return;
      }
    }

    // 4. MANDATORY QUESTIONS CHECK
    const questions = await Question.find({ pollId: poll._id, isMandatory: true });
    const answeredQuestionIds = answers.map((a: any) => a.questionId);
    
    for (const q of questions) {
      if (!answeredQuestionIds.includes(q._id.toString())) {
        res.status(400).json({ success: false, error: `Mandatory question missing: ${q.text}` });
        return;
      }
    }
