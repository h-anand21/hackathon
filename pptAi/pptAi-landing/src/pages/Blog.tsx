...
// Define types for Hashnode API response
...
export default function Blog() {
  ...
  useEffect(() => {
    async function fetchPosts() {
      try {
        ...
        const fetchedPosts = data.data?.publication?.posts?.edges || [];
  ...
  return (
    <main className="relative z-10 min-h-screen pt-32 pb-24 px-6 flex flex-col items-center">
      ...
      >
        <p className="text-indigo-600 font-bold text-sm uppercase tracking-widest mb-3">Our Thoughts</p>
              ...
              >
                ...
                <div className="p-6 flex flex-col flex-1">
                  <div className="flex items-center gap-4 text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">
                    <span>{date}</span>
                    <span className="flex items-center gap-1"><Clock className="w-3 h-3"/> {post.readTimeInMinutes} min read</span>
                  </div>
                  <h2 className="text-xl font-bold text-slate-900 mb-3 leading-snug group-hover:text-indigo-600 transition-colors line-clamp-2">
                    {post.title}
                  </h2>
